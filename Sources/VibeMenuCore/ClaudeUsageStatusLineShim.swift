...
/// The exact source of VibeMenu's opt-in Claude Code usage-limit statusLine shim
/// (docs/decisions/0016-claude-usage-limits.md).
///
/// Embedded as a constant so the in-app one-click installer can materialise the script to a stable
/// path (`~/Library/Application Support/VibeMenu/ClaudeUsage/vibemenu-usage-statusline.sh`) that does
/// not depend on the app bundle's location and survives app moves/updates — without needing a bundled
/// resource wired into both the SwiftPM executable and the Xcode `.app` target. It is byte-for-byte
/// identical to the canonical, human-readable copy at
/// `Support/ClaudeUsage/vibemenu-usage-statusline.sh` (used for manual install and code review); a
/// unit test asserts they never drift.
///
/// See `ClaudeUsageStatusLineShim` for what the script does and its hard privacy contract.
public enum ClaudeUsageStatusLineShim {
    /// The relative filename the installer writes and settings.json references.
    public static let fileName = "vibemenu-usage-statusline.sh"

    /// The full POSIX-sh script source.
